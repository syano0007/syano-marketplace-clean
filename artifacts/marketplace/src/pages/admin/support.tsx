...
function StatusBadge({ status }: { status: string }) {
  const config: Record<string, { label: string; cls: string }> = {
    ...
    resolved: { label: "Resolved", cls: "bg-emerald-500/10 text-emerald-600 border-emerald-200" },
      ...
      <div className="p-6 max-w-7xl mx-auto space-y-6">
        ...
        {stats ? (
          <div className="grid grid-cols-2 sm:grid-cols-4 gap-3">
            ...
            <StatCard label={t("support.stat_resolved")}  value={stats.resolved}  icon={CheckCircle2}   color="bg-emerald-500/10 text-emerald-600" />
                    ...
                    <div className="flex-1 min-w-0 space-y-1">
                      ...
                      <div className="flex items-center gap-3 flex-wrap text-xs text-muted-foreground">
                        ...
                        {ticket.assigned_admin_name && (
                          <span className="text-emerald-600">{t("support.assigned_to")}: {ticket.assigned_admin_name}</span>
                        )}
                      </div>
                    </div>
