...
function ConvTypeIcon({ type }: { type: string }) {
  ...
  if (type === "seller_admin") return <Store className="h-3.5 w-3.5 text-emerald-500" />;
...
function ConvTypeBadge({ type }: { type: string }) {
  ...
  const color = type === "customer_admin" ? "bg-blue-500/10 text-blue-600 border-blue-200"
    : type === "seller_admin" ? "bg-emerald-500/10 text-emerald-600 border-emerald-200"
      ...
      <div className="absolute end-2 top-2 opacity-0 group-hover:opacity-100 transition-opacity z-10">
        <DropdownMenu>
          ...
          <DropdownMenuContent align="end" className="w-40">
            <DropdownMenuItem onClick={onBlock} className={isBlocked ? "text-emerald-600" : "text-destructive"}>
              {isBlocked ? (
