...
function DatePicker({ preset, from, to, onPreset, onCustom, t, lang }: DatePickerProps) {
  ...
  return (
    <div className="relative">
      ...
      {open && (
        <div className={`absolute top-full mt-2 z-50 bg-popover border rounded-xl shadow-xl p-3 w-72 ${lang === "ar" ? "left-0" : "right-0"}`}>
              ...
              <div>
                <ResponsiveContainer width="100%" height={200}>
                  <AreaChart data={chart.points} margin={{ top: 4, right: 4, left: 0, bottom: 0 }}>
                    ...
                    <Area yAxisId="rev" type="monotone" dataKey="revenue" name={t("seller_analytics.chart_revenue")} stroke="#10b981" strokeWidth={2} fill="url(#gradRevenue)" dot={false} activeDot={{ r: 4, strokeWidth: 0 }} />
                  ...
                  <div className="mt-4 pt-4 border-t">
                    <ResponsiveContainer width="100%" height={160}>
                      <BarChart data={productBarData.slice(5, 10)} layout="vertical" margin={{ top: 0, right: 4, left: 0, bottom: 0 }}>
                        ...
                        <Bar dataKey="revenue" name={t("seller_analytics.chart_revenue")} fill="#10b981" radius={[0, 4, 4, 0]} barSize={12} />
                      </BarChart>
                    </ResponsiveContainer>
                  </div>
                )}
            ...
            {isLoading ? <Skeleton className="h-64 w-full" /> :
             !s || s.customers.unique === 0 ? (
              ...
              <div className="space-y-0">
                ...
                <StatRow icon={<UserCheck className="h-3.5 w-3.5" />}   label={t("seller_analytics.cust_returning")}   value={String(s.customers.returning)} accent="text-emerald-600" bar={(s.customers.returning / (s.customers.unique || 1)) * 100} barColor="#10b981" />
          ...
          <SectionCard title={t("seller_analytics.delivery_title")} subtitle={t("seller_analytics.delivery_sub")}>
            {isLoading ? <Skeleton className="h-52 w-full" /> :
             !s || (s.delivery.totalDelivered === 0 && s.delivery.totalFailed === 0) ? (
              ...
              <div className="space-y-0">
                <StatRow icon={<CheckCircle className="h-3.5 w-3.5" />} label={t("seller_analytics.dlv_delivered")}    value={String(s.delivery.totalDelivered)}  accent="text-emerald-600" bar={s.delivery.successRate} barColor="#10b981" />
