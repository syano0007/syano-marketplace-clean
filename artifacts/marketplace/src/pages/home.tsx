...
/* ── Category chip strip (8 quick-access chips, enhanced design) ── */

const QUICK_CATEGORIES = [
  { slug: "Electronics",            en: "Electronics", ar: "الإلكترونيات",   Icon: Cpu,           iconBg: "bg-blue-100 dark:bg-blue-900/40",    iconColor: "text-blue-600 dark:text-blue-400" },
  { slug: "Fashion",                en: "Fashion",     ar: "الأزياء",          Icon: Shirt,         iconBg: "bg-pink-100 dark:bg-pink-900/40",    iconColor: "text-pink-600 dark:text-pink-400" },
  { slug: "Home & Kitchen",         en: "Home",        ar: "المنزل",           Icon: HomeIcon,      iconBg: "bg-amber-100 dark:bg-amber-900/40",  iconColor: "text-amber-600 dark:text-amber-400" },
  { slug: "Beauty & Personal Care", en: "Beauty",      ar: "الجمال",           Icon: Sparkles,      iconBg: "bg-rose-100 dark:bg-rose-900/40",    iconColor: "text-rose-600 dark:text-rose-400" },
  { slug: "Sports & Fitness",       en: "Sports",      ar: "الرياضة",          Icon: Dumbbell,      iconBg: "bg-green-100 dark:bg-green-900/40",  iconColor: "text-green-600 dark:text-green-400" },
  { slug: "Automotive",             en: "Automotive",  ar: "السيارات",         Icon: Car,           iconBg: "bg-slate-100 dark:bg-slate-800",     iconColor: "text-slate-600 dark:text-slate-400" },
  { slug: "Gaming & Entertainment", en: "Gaming",      ar: "الألعاب",          Icon: Gamepad2,      iconBg: "bg-violet-100 dark:bg-violet-900/40",iconColor: "text-violet-600 dark:text-violet-400" },
  { slug: "Supermarket & Grocery",  en: "Grocery",     ar: "البقالة",          Icon: ShoppingBasket,iconBg: "bg-emerald-100 dark:bg-emerald-900/40",iconColor: "text-emerald-600 dark:text-emerald-400" },
] as const;
    ...
    <div className="border-b bg-background">
      <div className="container px-4">
        <div className="flex items-center gap-2 overflow-x-auto scrollbar-hide py-3">
          {QUICK_CATEGORIES.map(({ slug, en, ar, Icon, iconBg, iconColor }) => (
            <Link
              key={slug}
              href={`/products?category=${encodeURIComponent(slug)}`}
              className="shrink-0"
            >
              <div className="flex items-center gap-2 px-3.5 py-2 rounded-xl border border-border hover:border-primary/40 bg-background hover:bg-muted transition-all duration-150 hover:shadow-sm group cursor-pointer">
                <div
                  className={`h-6 w-6 rounded-lg flex items-center justify-center shrink-0 ${iconBg}`}
                >
          ...
          <Link href="/products" className="shrink-0">
            <div className="flex items-center gap-1.5 px-3.5 py-2 rounded-xl border border-dashed border-border hover:border-primary/40 bg-background hover:bg-muted transition-all duration-150 cursor-pointer">
  ...
  return (
    <Layout>
      <div className="w-full">
        ...
        {/* ════════════════════════════════════════════════════
            CATEGORY CHIP ROW — 8 quick-access departments
        ════════════════════════════════════════════════════ */}
        <CategoryChipRow />
        ...
        {/* ════════════════════════════════════════════════════
            CATEGORY GALLERY — large photo cards, all departments
        ════════════════════════════════════════════════════ */}
        <section
          id="categories"
          className="py-12 md:py-16 border-b bg-muted/15 cv-section"
        >
          <div className="container px-4">
            <SectionHeader
            ...
            <div className="category-grid">
              {CATEGORIES.map((cat) => {
                ...
                return (
                  ...
                  >
                    <div className="relative group overflow-hidden rounded-2xl border border-border/50 hover:border-primary/50 cursor-pointer aspect-[4/3] shadow-sm hover:shadow-md transition-[transform,border-color,box-shadow] duration-200 hover:-translate-y-0.5 active:scale-[0.97]">
                      {img ? (
                        <img
                          src={img}
                          alt={cat.en}
                        ...
                        <div
                          className={`absolute inset-0 ${cat.iconBg}`}
                        />
                      )}
                      ...
                      <div className="absolute top-2.5 end-2.5 z-10">
                        <div
                          className={`h-7 w-7 sm:h-8 sm:w-8 rounded-full ${cat.iconBg} flex items-center justify-center shrink-0 shadow-md ring-1 ring-white/25`}
                        >
                          <Icon
                            className={`h-3.5 w-3.5 sm:h-4 sm:w-4 ${cat.iconColor}`}
                          />
                        </div>
                      </div>
