...
import {
  Bot, X, Send, Sparkles, ExternalLink, RefreshCw,
} from "lucide-react";
  ...
  return (
    ...
    >
      ...
      <button
        onClick={() => setOpen((o) => !o)}
        className={`
          relative flex items-center gap-2 rounded-2xl px-4 py-2.5 shadow-lg transition-all duration-200
          ${open
      ...
      >
        ...
        <span className="text-sm font-semibold whitespace-nowrap hidden sm:inline">
          {t("support.agent_name")}
        </span>
        {!open && (
          <Sparkles className="h-3.5 w-3.5 opacity-70 hidden sm:inline shrink-0" />
        )}

        {/* Unread badge */}
        {unreadCount > 0 && !open && (
          <span
            className="absolute -top-1.5 -end-1.5 flex h-5 w-5 items-center justify-center rounded-full bg-red-500 text-[10px] font-bold text-white shadow-sm"
