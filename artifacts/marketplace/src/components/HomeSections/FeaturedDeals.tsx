...
const STATIC_DEALS = [
  { id: 1, name: "ساعة كلاسيكية ذهبية", category: "ساعات فاخرة", price: "142,500", originalPrice: "237,000", discount: 40, rating: 4.9, reviews: 284, img: "https://images.unsplash.com/photo-1772949399808-7020b02896b9?w=400&h=400&fit=crop&auto=format&q=85", badge: "الأكثر مبيعاً", badgeColor: "#f59e0b" },
  { id: 2, name: "حذاء نايكي رياضي", category: "رياضة وأحذية", price: "58,000", originalPrice: "82,000", discount: 29, rating: 4.7, reviews: 512, img: "https://images.unsplash.com/photo-1585232004423-244e0e6904e3?w=400&h=400&fit=crop&auto=format&q=85", badge: "عرض محدود", badgeColor: "#10b981" },
  { id: 3, name: "مجموعة تقنية متكاملة", category: "إلكترونيات", price: "385,000", originalPrice: "550,000", discount: 30, rating: 4.8, reviews: 196, img: "https://images.unsplash.com/photo-1741851547702-cac24b2a0d13?w=400&h=400&fit=crop&auto=format&q=85", badge: "جديد", badgeColor: "#3b82f6" },
  { id: 4, name: "عطر أوبسيديان إليكسير", category: "عطور وجمال", price: "96,000", originalPrice: "148,000", discount: 35, rating: 4.6, reviews: 89, img: "https://images.unsplash.com/photo-1772191399367-91ed8d95664b?w=400&h=400&fit=crop&auto=format&q=85", badge: "حصري", badgeColor: "#8b5cf6" },
];
      ...
      <div className="flex items-center gap-1">
        {[pad(time.h), pad(time.m), pad(time.s)].map((val, i) => (
          <span key={i} className="flex items-center gap-1">
            <span style={{ fontWeight: 700, fontSize: "14px", fontVariantNumeric: "tabular-nums" }} className="bg-[#1a1a1a] text-white px-2 py-0.5 rounded-md min-w-[32px] text-center">
              {val}
            </span>
...
function DealCard({ deal, i }: { deal: DealCardData; i: number }) {
  return (
    <motion.div
      ...
      className="group bg-[#0f0f0f] border border-white/[0.06] hover:border-white/[0.1] rounded-2xl overflow-hidden transition-all duration-300 hover:shadow-2xl hover:shadow-black/60 hover:-translate-y-1 cursor-pointer"
    >
      ...
      <div className="p-5">
        ...
        <div className="flex items-center justify-between">
          ...
          <button style={{ fontWeight: 600, fontSize: "13px" }} className="bg-white/[0.06] hover:bg-emerald-500/10 hover:text-emerald-400 text-white/60 px-3.5 py-2 rounded-xl transition-all duration-200 border border-white/[0.06] hover:border-emerald-500/30">
            أضف
          </button>
        </div>
      </div>
...
export function FeaturedDeals({ hotDeals }: { hotDeals?: Product[] }) {
  const deals: DealCardData[] = hotDeals && hotDeals.length >= 4
    ...
    <section style={{ direction: "rtl", fontFamily: "'Cairo', sans-serif" }} className="bg-[#080808] py-28 border-t border-white/[0.04]">
      <div className="max-w-[1400px] mx-auto px-10">
        ...
        <div className="grid grid-cols-4 gap-5">
          {deals.map((deal, i) => <DealCard key={deal.id} deal={deal} i={i} />)}
        </div>
      </div>
    </section>
  );
