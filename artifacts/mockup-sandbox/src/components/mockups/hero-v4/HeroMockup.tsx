...
/* ─────────────────────────────────────────────────────────────────
   PRECISE COLOR TOKENS — sampled from approved reference screenshot
   Background:   #080808  (richer/deeper than #0a0a0a)
   Green:        #10b981  (confirmed emerald-500, unchanged)
   Muted text:   #5a626e  (slightly darker than Tailwind gray-500)
   Border:       rgba(255,255,255,0.075)
   Card bg:      rgba(6,6,6,0.93)
   Grid dots:    rgba(255,255,255,0.024) — barely perceptible texture
───────────────────────────────────────────────────────────────── */
const GLOBAL_CSS = `
  ...
  *, *::before, *::after { box-sizing: border-box; margin: 0; padding: 0; }
  ...
  /* dot-grid — density + opacity matched to reference */
  .hm-root {
...
/* ── Floating card — precise glass match ─────────────────── */
function FloatCard({ card }: { card: Card }) {
  return (
    <div style={{
      ...
      zIndex: 10,
      width: card.w,
      /* Card bg: deep dark glass — sampled from reference */
      ...
      gap: 14,
      alignItems: "center",
      /* Shadow: stronger/richer than before to match premium reference */
      boxShadow: "0 18px 52px rgba(0,0,0,0.84), 0 4px 16px rgba(0,0,0,0.60)",
      animation: card.anim,
      direction: "rtl",
    }}>
      <div style={{ flex: 1, textAlign: "right" }}>
        <div style={{ fontSize: 11, color: "#5a626e", marginBottom: 4 }}>
          {card.label}
        </div>
        <div style={{
          fontSize: 16, fontWeight: 800, color: "#f3f3f3",
          marginBottom: (card.stars != null || card.avail) ? 6 : 0,
        }}>
          {card.price}{" "}
...
function SlideLayer({ slide, visible }: { slide: typeof SLIDES[0]; visible: boolean }) {
  return (
    <div style={{
      ...
      {/* Dark overlay — stronger than before for premium depth */}
      <div style={{
        position: "absolute", inset: 0,
        background: "rgba(0,0,0,0.35)",
      }} />

      {/* Ground-plane gradient — rich dark bottom */}
      <div style={{
        position: "absolute", inset: "auto 0 0 0", height: "40%",
        background: "linear-gradient(to top, rgba(0,0,0,0.78) 0%, transparent 100%)",
      }} />

      {/* Blend edge — right side fades into text panel (#080808) */}
      <div style={{
      ...
      {/* Discount badge — strong green glow */}
      <div style={{
        ...
        /* Stronger glow matching reference */
...
/* ════════════════════════════════════════════════════════════
   HeroMockup — visual fidelity pass (colors, shadows, type)
════════════════════════════════════════════════════════════ */
export function HeroMockup() {
  ...
  return (
    <div className="hm-root" dir="rtl">
      ...
      {/* ── Navbar — deep dark, crisp border ──────────────── */}
      <div style={{
        position: "fixed", top: 0, left: 0, right: 0, height: 58,
        /* Navbar: even deeper than page bg */
        ...
        <div style={{
          ...
          <span style={{ fontSize: 12, color: "#3d454e" }}>
            ابحث عن منتجات، متاجر أو فئات...
          </span>
        </div>
        <div style={{ display: "flex", gap: 10, alignItems: "center" }}>
          <button style={{
            ...
            color: "#9ca3af", fontSize: 12, padding: "8px 18px",
              ...
              {SLIDES.map((_, i) => (
                <button
                  ...
                  style={{
                    ...
                    borderRadius: 100,
                    transition: "all 0.3s",
                    /* Active dot: solid green; inactive: dim white */
            ...
            <div style={{
              ...
              <div>
                <span style={{
                  ...
                  /* Badge: subtle green tint, crisp border */
              ...
              {/* ── HEADLINE — tight tracking, heavy weight ── */}
              <h1 style={{
                margin: 0,
                fontSize: "clamp(48px, 5.5vw, 80px)",
                fontWeight: 900,
                /* Tighter letter spacing — matches reference rendering */
                lineHeight: 1.02,
                letterSpacing: "-2.5px",
                color: "#f3f3f3",
              }}>
                اكتشف آلاف<br />
                المنتجات من<br />
                {/* Green exactly matches #10b981 from reference */}
              ...
              <div style={{ display: "flex", gap: 14, alignItems: "center", flexWrap: "wrap" }}>
                <a href="#" style={{
                  ...
                  textDecoration: "none",
                  /* Stronger glow to match reference premium feel */
              ...
              <div style={{
                ...
                {STATS.map((s, i) => (
                  <div key={s.l} style={{
                    ...
                    <div style={{ fontSize: 12, marginTop: 5, color: "#5a626e" }}>
                      {s.l}
                    </div>
                  </div>
                ))}
