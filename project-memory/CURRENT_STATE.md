...
1. Read `RECOVERY_GUIDE.md` for the complete step-by-step recovery procedure.
2. Read `DATABASE_STATE.md` for schema details.
3. Read `API_STATE.md` for API surface.
4. Verify this checkpoint matches the above state before making any changes.
5. DO NOT rebuild or redesign anything without a proven defect.
6. The official logo is at `artifacts/marketplace/src/assets/syano-logo.png` — always restore from there.
